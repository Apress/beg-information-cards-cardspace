...
namespace BeginningCardSpace
{
    public class ManagedCardHelper
    {
        ...
        public static X509Certificate2 RetrieveCertificate(string certDetails)
        {
            CertificateInfo certInfo = new CertificateInfo();
            string[] splitCertDetails = certDetails.Split("/".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            certInfo.Location = splitCertDetails[0];
            certInfo.Store = splitCertDetails[1];
            certInfo.CommonName = splitCertDetails[2];
            return RetrieveCertificate(certInfo);
        
        
        }
        
        public static X509Certificate2 RetrieveCertificate(CertificateInfo certInfo)
        {
            ...
            try
            {
                storeLocation = (StoreLocation)Enum.Parse(typeof(StoreLocation), certInfo.Location, true);
            }
            catch (Exception)
            {
                throw new Exception("No Certificate Location: " + certInfo.Location);
            }
            try
            {
                storeName = (StoreName)Enum.Parse(typeof(StoreName), certInfo.Location, true);
            }
            catch (Exception)
            {
                throw new Exception("No Certificate Store: " + certInfo.Location + " in " + certInfo.Store);
            }

            X509Store s = new X509Store(storeName, storeLocation);
            s.Open(OpenFlags.MaxAllowed);
            foreach (X509Certificate2 thisCert in s.Certificates)
            {
                if (thisCert.Subject.StartsWith("CN=" + certInfo.CommonName))
                {
        ...
        {
            System.Xml.Serialization.XmlSerializer xmlSerializer = new System.Xml.Serialization.XmlSerializer(card.GetType());
            XmlTextWriter xmlTextWriter = new XmlTextWriter(templateFilename, Encoding.UTF8);
            xmlSerializer.Serialize(xmlTextWriter, card);
            xmlTextWriter.Flush();
            xmlTextWriter.Close();
        ...
        public XmlElement SignInformationCardXML(XmlDocument doc, X509Certificate2 cert)
        {
        ...
        public void SaveInformationCard(XmlElement signedInformationCard, string filename)
        {

            XmlTextWriter textWriter = new XmlTextWriter(filename, Encoding.UTF8);
            signedInformationCard.WriteTo(textWriter);
